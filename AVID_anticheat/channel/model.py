...
class ActiveFraud(object):
    ...
    def insertScore(self):
        ...
        for row in raw:
            ...
            score = reduce(lambda x,y:x*y,scoreList) * 2 ** (len(row)-3) / 15 * 100 * row[3] * row[3] * row[4] * row[5]
    ...
    def analysis(self):
        ...
        dfImsi = self.makeConfidenceIntervalEstimation(imsi, ['imsi_rate'], rate=0.3)

        imei = pd.concat([rawChannel, rawImei], axis=1)
        dfImei = self.makeConfidenceIntervalEstimation(imei, ['imei_rate'], rate=0.3)
...
if __name__ == '__main__':
    ...
    #af = ActiveFraud('600025', '2017-02-17', '2017-02-18')
    #rawChannel,rawIP,rawNetwork,rawHour,rawDevice = af.analysis()
    #result = af.analysis()
