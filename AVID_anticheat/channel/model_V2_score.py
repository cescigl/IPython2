...
class ActiveFraud(object):
    ...
    def __init__(self, ptid=None, begin=None, end=None):
        ...
        #构建数据查询sql
        self.sql_channel_cnt = "select media_source,count(distinct token) from view_appsflyer_postback_android_install_token where install_time>='" + self.yesterday + "' and install_time<'" + self.today + "' and product_id=" + self.ptid + " and token is not null group by media_source;"
        ...
        self.sql_channel_hour_cnt = "select media_source,date_part(hour, install_time),count(distinct token) from view_appsflyer_postback_android_install_token where install_time>='" + self.yesterday + "' and install_time<'" + self.today + "' and product_id=" + self.ptid + " and token is not null  group by media_source,date_part(hour, install_time);"
        ...
        self.sql_channel_device_cnt = "select media_source,device_brand,count(distinct token) from view_appsflyer_postback_android_install_token where install_time>='" + self.yesterday + "' and install_time<'" + self.today + "' and product_id=" + self.ptid + " and token is not null  group by media_source,device_brand;"
        ...
        self.sql_channel_date = "select distinct media_source,'" + self.yesterday + "','" + self.ptid + "' from view_appsflyer_postback_android_install_token where install_time>='" + self.yesterday + "' and install_time<'" + self.today + "' and product_id=" + self.ptid + " and token is not null;"
        #构建t分布预测模型
    ...
    def insertLoyaltyUser(self, ):
        ...
        sql_loyalty = "select install_time,media_source,product_id,sum(uncheat) as loyuser,sum(uncheat)*1.0/count(*) as loyrate from (select install_time,product_id,token,media_source, case when cnt_fg*1.0/cnt_all<0.6 then 0 when cnt_all<=10 then 0 else 1 end as uncheat from (select install_time,product_id,token,media_source, sum(cnt_key) as cnt_all, sum(case t3.sta_key when 'P' then cnt_key else 0 end) as cnt_fg from  (select trunc(t1.install_time) as install_time,product_id,t1.token,t1.media_source,t2.sta_key,count(*) as cnt_key from (select media_source,product_id,token,install_time from view_appsflyer_postback_android_install_token where  install_time>='" + self.yesterday + "' and install_time<'" + self.today + "' and product_id=" + self.ptid + ") t1 left outer join (select token,substring(sta_key,1,1) as sta_key,server_time from sta_event_game_publish where product_id=" + self.ptid + " and server_time>='" + self.yesterday + "' and server_time<'" + self.today + "') t2 on t1.token=t2.token group by trunc(t1.install_time),product_id,t1.token,t1.media_source,t2.sta_key) t3  group by install_time,product_id,token,media_source ) t4 ) t5 group by install_time,product_id,media_source;"

        sql_loyalty = "select install_time,media_source,product_id,sum(uncheat) as loyuser,sum(uncheat)*1.0/count(*) as loyrate from (select install_time,product_id,token,media_source, case when cnt_all>=100 then 1 when cnt_bg*1.0/cnt_all>0.4 then 0 else 1 end as uncheat from (select install_time,product_id,token,media_source, sum(cnt_key) as cnt_all, sum(case t3.sta_key when '_' then cnt_key else 0 end) as cnt_bg from  (select trunc(t1.install_time) as install_time,product_id,t1.token,t1.media_source,t2.sta_key,count(*) as cnt_key from (select media_source,product_id,token,install_time from view_appsflyer_postback_android_install_token where  install_time>='" + self.yesterday + "' and install_time<'" + self.today + "' and product_id=" + self.ptid + ") t1 left outer join (select token,substring(sta_key,1,1) as sta_key,server_time from sta_event_game_publish where product_id=" + self.ptid + " and server_time>='" + self.yesterday + "' and server_time<'" + self.today + "') t2 on t1.token=t2.token group by trunc(t1.install_time),product_id,t1.token,t1.media_source,t2.sta_key) t3  group by install_time,product_id,token,media_source ) t4 ) t5 group by install_time,product_id,media_source;"
