...
class ActiveFraud(object):
    ...
    def __init__(self, begin=None, end=None):
        ...
        #构建数据查询sql
        self.sql_channel_cnt = "select product_id,media_source,count(distinct token) from view_appsflyer_postback_ios_install_token where install_time>='" + self.yesterday + "' and install_time<'" + self.today + "' and product_id in (select product_id from view_appsflyer_postback_ios_install_token where install_time>='" + self.yesterday + "' and install_time<'" + self.today + "' and product_id>600000  and token is not null group by product_id having(count(distinct media_source)>1)) and token is not null group by media_source,product_id;"

        self.sql_channel_ip_cnt = "select product_id,media_source,sum(cnt) from (select product_id,media_source,ip,count(distinct token) as cnt from view_appsflyer_postback_ios_install_token where install_time>='" + self.yesterday + "' and install_time<'" + self.today + "' and product_id in (select product_id from view_appsflyer_postback_ios_install_token where install_time>='" + self.yesterday + "' and install_time<'" + self.today + "' and product_id>600000  and token is not null group by product_id having(count(distinct media_source)>1)) and token is not null group by product_id,media_source,ip having count(distinct token)<5) group by product_id,media_source;"
        ...
        self.sql_channel_imsi_cnt = "select product_id,media_source,sum(case when lenimsi=15 then 1 else 0 end) as cnt from (select distinct product_id,media_source,token,len(split_part(imsi,'_',2)) as lenimsi from view_appsflyer_postback_ios_install_token where install_time>='" + self.yesterday + "' and install_time<'" + self.today + "' and  product_id in (select product_id from view_appsflyer_postback_ios_install_token where install_time>='" + self.yesterday + "' and install_time<'" + self.today + "' and product_id>600000  and token is not null group by product_id having(count(distinct media_source)>1)) and token is not null ) t group by product_id,media_source;"

        self.sql_channel_imei_cnt = "select product_id,media_source,sum(case when lenimei=15 then 1 else 0 end) as cnt from (select distinct product_id,media_source,token,len(imei) as lenimei from view_appsflyer_postback_ios_install_token where install_time>='" + self.yesterday + "' and install_time<'" + self.today + "' and product_id in (select product_id from view_appsflyer_postback_ios_install_token where install_time>='" + self.yesterday + "' and install_time<'" + self.today + "' and product_id>600000  and token is not null group by product_id having(count(distinct media_source)>1)) and token is not null ) t group by product_id,media_source;"

        self.sql_channel_hour_cnt = "select product_id,media_source,date_part(hour, install_time),count(distinct token) from view_appsflyer_postback_ios_install_token where install_time>='" + self.yesterday + "' and install_time<'" + self.today + "' and token is not null and product_id in (select product_id from view_appsflyer_postback_ios_install_token where install_time>='" + self.yesterday + "' and install_time<'" + self.today + "' and product_id>600000  and token is not null group by product_id having(count(distinct media_source)>1))group by product_id,media_source,date_part(hour, install_time);"
        ...
        self.sql_channel_device_cnt = "select product_id,media_source,device_brand,count(distinct token) from view_appsflyer_postback_ios_install_token where install_time>='" + self.yesterday + "' and install_time<'" + self.today + "' and product_id in (select product_id from view_appsflyer_postback_ios_install_token where install_time>='" + self.yesterday + "' and install_time<'" + self.today + "' and product_id>600000  and token is not null group by product_id having(count(distinct media_source)>1)) and token is not null group by product_id,media_source,device_brand;"
        ...
        self.sql_channel_date = "select distinct product_id,media_source,'" + self.yesterday + "' from view_appsflyer_postback_ios_install_token where install_time>='" + self.yesterday + "' and install_time<'" + self.today + "' and product_id>600000 and token is not null and product_id in (select product_id from view_appsflyer_postback_ios_install_token where install_time>='" + self.yesterday + "' and install_time<'" + self.today + "' and product_id>600000  and token is not null group by product_id having(count(distinct media_source)>1));"
    ...
    def insertScore(self):
        ...
        sql_new = "select day,channel,product_id,ifnull(loyalty_users_rate,0),ifnull(retain,0),ifnull(avg_return_7day,0),ifnull(net_rate,0),ifnull(hour_rate,0),ifnull(device_rate,0) from active_fraud_analysis where install>50 and day>='"+self.date_7day_ago+"' and day<'"+self.today+"' ;"
    ...
    def analysisHour(self):
        ...
        for p in product_set:
            ...
            organicMost = [rawOrganic[i][2] for i in range(min(3,len(rawOrganic)))]
            #分别求总数和前三个月聚合累加
            raw3most = [(i[0],i[1],i[3]) for i in raw if i[2] in organicMost and i[0] == p]
            #rawAll = [(i[1],i[3]) for i in raw]
            
            df3most = self.makeDF(raw3most, False, ['key_p', 'key_m', 'cnt_hour'])
    ...
    def makeConfidenceRate(self, df, col, tag=False, rate=0.3):
        """
        tag表示是否是单边检测，还是中心检测，如果为True，且是单边检测，检测数据比率大于organic数据，可信度为1。默认是False
        """
        dfData = df.fillna(0).reset_index().values
        ...
        pdf = pd.DataFrame()
        for p in product_set:
            u0 = [i[3]*1.0/i[2] for i in dfData if i[1] == 'Organic' and i[0]==p][0]
            
            dfList = [[i[0], i[1], i[2], u0, i[3]*1.0/i[2], tag, rate] for i in dfData if i[0]==p]
    ...
    def saveToMysql(self, df):
        ...
        if len(data[0]) == 16:
            ...
            values =[(i[0][0],i[0][1],i[1],i[2],i[3],i[4],i[5],i[6],i[7],i[8],i[9],i[10],i[11],i[12],i[13],i[14]) for i in data]
    ...
    def analysis(self):
        ...
        print 'BEGIN'
        rawData = self.analysisModel(self.sql_channel_date, False, ['key_p', 'key_m', 'datetime']) # 日期
        print 'rawData'
        rawData = rawData.groupby(['key_p', 'key_m']).max()

        rawChannel = self.analysisModel(self.sql_channel_cnt, False, ['key_p', 'key_m', 'channel']) #总数
        print 'rawChannel'
        rawChannel = rawChannel.groupby(['key_p', 'key_m']).max()
        print rawChannel
        rawIP = self.analysisModel(self.sql_channel_ip_cnt, False, ['key_p', 'key_m', 'cnt_IP'])    #IP重复分布
        rawIP = rawIP.groupby(['key_p', 'key_m']).max()
        print 'rawIP'
        print rawIP

        rawNetwork = self.analysisModel(self.sql_channel_wifi_cnt, False, ['key_p', 'key_m', 'cnt_Net'])    #网络分布
        rawNetwork = rawNetwork.groupby(['key_p', 'key_m']).max()
        print 'rawNetwork'
        #rawIPseg = self.analysisModel(self.sql_channel_IPseg_cnt, True, ['cnt_IPseg'])    #IP段分布
        rawImsi = self.analysisModel(self.sql_channel_imsi_cnt, False, ['key_p', 'key_m', 'cnt_IMSI']) #IMSI为空比率
        print 'rawImsi'
        rawImsi = rawImsi.groupby(['key_p', 'key_m']).max()
        print rawImsi
        rawImei = self.analysisModel(self.sql_channel_imei_cnt, False, ['key_p', 'key_m', 'cnt_IMEI']) #IMEI为空比率
...
if __name__ == '__main__':
    ...
    af = ActiveFraud(args.begin, args.end)
    af.analysis()
    #af.insertRetain()
    #af.insertAvgReturn7day()
    #af.insertLoyaltyUser()
    #af.insertScore()
    #af.modifyScore()
