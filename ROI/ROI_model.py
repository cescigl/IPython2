...
class ARIMAModel(baseModel):
    ...
    def __init__(self, tablename, whereSql, predictDays, beginDay, endDay, colname):
        ...
        if self.table == "ROI_Retained_Predict_GroupBy_c_his":
            self.keyList = [0,2,3,4,5]
            self.valueList = [1,6]
            if self.colname == "retain":
                self.insertSql = "INSERT INTO ROI_Retained_Predict_GroupBy_c_his (`date`, retain_day, pdtid, country_iso,channel_category, pid, retain_predict) VALUES (%s,%s,%s,%s,%s,%s,%s) ON DUPLICATE KEY UPDATE retain_predict=%s;"
                self.selectSql = "select date,retain_day,pdtid,country_iso,channel_category,pid,if(1/retain,retain,retain_predict)  as retain from ROI_Retained_Predict_GroupBy_c_his where " + whereSql + " and retain_day<" + endDay + " and retain_day>="+ beginDay +";"
            elif self.colname == "spec_retain":
                self.insertSql = "INSERT INTO ROI_Retained_Predict_GroupBy_c_his (`date`, retain_day, pdtid, country_iso,channel_category, pid, spec_retain_predict) VALUES (%s,%s,%s,%s,%s,%s,%s) ON DUPLICATE KEY UPDATE spec_retain_predict=%s;"
                self.selectSql = "select date,retain_day,pdtid,country_iso,channel_category,pid,if(1/spec_retain,spec_retain,spec_retain_predict)  as spec_retain from ROI_Retained_Predict_GroupBy_c_his where " + whereSql + " and retain_day<" + endDay + " and retain_day>="+ beginDay +";"
            
        elif self.table == "ROI_Purchase_Predict_GroupBy_c_his":
            self.keyList = [0,2,3,4,5]
            self.valueList = [1,6]
            self.insertSql = "INSERT INTO ROI_Purchase_Predict_GroupBy_c_his (`date`,revenue_day,pdtid,country_iso,channel_category,pid,purchase_predict) VALUES (%s,%s,%s,%s,%s,%s,%s) ON DUPLICATE KEY UPDATE purchase_predict=%s;"
            self.selectSql = "select date,revenue_day,pdtid,country_iso,channel_category,pid,if(1/purchase,purchase,purchase_predict)  as purchase from ROI_Purchase_Predict_GroupBy_c_his where " + whereSql + " and revenue_day<" + endDay + " and revenue_day>="+ beginDay +";"
            
        elif self.table == "ROI_News_Predict_GroupBy_c_his":
            self.keyList = [0,2,3,4,5,6,7]
            self.valueList = [1,8]

            if self.colname == "imp_num":
                self.insertSql = "INSERT INTO ROI_News_Predict_GroupBy_c_his (`date`,revenue_date,pdtid,country_iso,channel_category,pid,ad_type_id,ad_type,imp_num_predict) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s) ON DUPLICATE KEY UPDATE imp_num_predict=%s;"

                self.selectSql = "select `date`,revenue_date,pdtid,country_iso,channel_category,pid,ad_type_id,ad_type,if(1/imp_num,imp_num,imp_num_predict) as imp_num from ROI_News_Predict_GroupBy_c_his where " + whereSql + " and revenue_date<" + endDay + " and revenue_date>="+ beginDay +";"
            elif self.colname == "click_num":
                self.insertSql = "INSERT INTO ROI_News_Predict_GroupBy_c_his (`date`,revenue_date,pdtid,country_iso,channel_category,pid,ad_type_id,ad_type,click_num_predict) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s) ON DUPLICATE KEY UPDATE click_num_predict=%s;"
                self.selectSql = "select `date`,revenue_date,pdtid,country_iso,channel_category,pid,ad_type_id,ad_type,if(1/click_num,click_num,click_num_predict) as click_num from ROI_News_Predict_GroupBy_c_his where " + whereSql + " and revenue_date<" + endDay + " and revenue_date>="+ beginDay +";"
            elif self.colname == "imp_revenue":
                self.insertSql = "INSERT INTO ROI_News_Predict_GroupBy_c_his (`date`,revenue_date,pdtid,country_iso,channel_category,pid,ad_type_id,ad_type,imp_revenue_predict) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s) ON DUPLICATE KEY UPDATE imp_revenue_predict=%s;"
                self.selectSql = "select `date`,revenue_date,pdtid,country_iso,channel_category,pid,ad_type_id,ad_type,if(1/imp_revenue,imp_revenue,imp_revenue_predict) as imp_revenue from ROI_News_Predict_GroupBy_c_his where " + whereSql + " and revenue_date<" + endDay + " and revenue_date>="+ beginDay +";"
            elif self.colname == "click_revenue":
                self.insertSql = "INSERT INTO ROI_News_Predict_GroupBy_c_his (`date`,revenue_date,pdtid,country_iso,channel_category,pid,ad_type_id,ad_type,click_revenue_predict) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s) ON DUPLICATE KEY UPDATE click_revenue_predict=%s;"
                self.selectSql = "select `date`,revenue_date,pdtid,country_iso,channel_category,pid,ad_type_id,ad_type,if(1/click_revenue,click_revenue,click_revenue_predict) as click_revenue from ROI_News_Predict_GroupBy_c_his where " + whereSql + " and revenue_date<" + endDay + " and revenue_date>="+ beginDay +";"
            else:
            ...
            if length>10:
                ...
                for i in range(self.forecastnum):
                    ...
                    if self.table == "ROI_Retained_Predict_GroupBy_c_his":
                        ...
                        resultValues = [keys[0], dateStr, keys[1], keys[2], keys[3], keys[4], result[i], result[i]]
                    elif self.table == "ROI_install_Predict_GroupBy_c_his":
...
class RegModel(baseModel):
    ...
    def __init__(self, tablename, predictDays, colname, queue):
        ...
        if self.table == "ROI_Retained_Predict_GroupBy_c_his":
            self.exeSql = "INSERT INTO ROI_Retained_Predict_GroupBy_c_his (`date`, retain_day, pdtid, country_iso, channel_category, pid, retain, spec_retain) select date,retain_day,pdtid,country_iso,channel_category,pid,retain,spec_retain from (select date,retain_day,pdtid,country_iso,channel_category,pid,sum(retain) as retain,sum(spec_retain) as spec_retain from ROI_Retained where date>="+ self.daysAgo4 +" group by date,retain_day,pdtid,country_iso,channel_category,pid) t on DUPLICATE KEY UPDATE retain=t.retain,spec_retain=t.spec_retain;"
            ...
            if self.colname == "retain":
                self.insertSql = "INSERT INTO ROI_Retained_Predict_GroupBy_c_his (`date`, retain_day, pdtid, country_iso,channel_category, pid,retain_predict) VALUES (%s,%s,%s,%s,%s,%s,%s) ON DUPLICATE KEY UPDATE retain_predict=%s;"
                #获取经过处理的mysql数据
                self.selectSql = "select * from (select a.date,a.pdtid,a.country_iso,a.channel_category,a.pid,datediff(str_to_date(a.retain_day,'%Y%m%d'),str_to_date(b.retain_day,'%Y%m%d')) as day_diff,b.retain_day as retain_day_min,log(b.retain/a.retain) as log_retain_rate,b.retain from (select * from ROI_Retained_Predict_GroupBy_c_his where retain_day<" + self.today + " and retain>0 and date>=" + self.daysAgo60 + ") a,(select date,min(retain_day) as retain_day,pdtid,country_iso,channel_category,pid,retain as retain from ROI_Retained_Predict_GroupBy_c_his where retain_day<" + self.today + " and retain>0 and date>=" + self.daysAgo60 + " group by date,pdtid,country_iso,channel_category,pid) b where a.date=b.date and a.pdtid=b.pdtid and a.country_iso=b.country_iso and a.channel_category=b.channel_category and a.pid=b.pid) d where day_diff>0 and log_retain_rate is not null;"
                #调试sql
                #self.selectSql = "select * from (select a.date,a.pdtid,a.country_iso,a.channel_category,a.pid,datediff(str_to_date(a.retain_day,'%Y%m%d'),str_to_date(b.retain_day,'%Y%m%d')) as day_diff,b.retain_day as retain_day_min,log(b.retain/a.retain) as log_retain_rate,b.retain from (select * from ROI_Retained_Predict_GroupBy_c_his where retain_day<" + self.today + " and pdtid='600027' and country_iso='US' and pid='Facebook Ads' and retain>0) a,(select date,min(retain_day) as retain_day,pdtid, country_iso,channel_category,pid,retain as retain from ROI_Retained_Predict_GroupBy_c_his where retain_day<" + self.today + " and retain>0 and date>=" + self.daysAgo60 + " group by date,pdtid,country_iso,channel_category,pid) b where a.date=b.date and a.pdtid=b.pdtid and a.country_iso=b.country_iso and a.channel_category=b.channel_category and a.pid=b.pid) d where day_diff>0 and log_retain_rate is not null;"
        
                self.predictSelectSql = "select b.date,b.retain_day,b.pdtid,b.country_iso,b.channel_category,b.pid,b.retain,datediff(str_to_date(b.retain_day,'%Y%m%d'),str_to_date(b.date,'%Y%m%d')) as day_diff from (select date,max(retain_day) as retain_day,pdtid,country_iso,channel_category,pid from ROI_Retained_Predict_GroupBy_c_his where retain_day<" + self.today + " and retain>0 and date>=" + self.daysAgo60 + " group by date,pdtid,country_iso,channel_category,pid) a,ROI_Retained_Predict_GroupBy_c_his b where a.date=b.date and a.pdtid=b.pdtid and a.country_iso=b.country_iso and a.channel_category=b.channel_category and a.pid=b.pid and a.retain_day=b.retain_day;"
            elif self.colname == "spec_retain":
                self.insertSql = "INSERT INTO ROI_Retained_Predict_GroupBy_c_his (`date`, retain_day, pdtid, country_iso,channel_category, pid,spec_retain_predict) VALUES (%s,%s,%s,%s,%s,%s,%s) ON DUPLICATE KEY UPDATE spec_retain_predict=%s;"
                #获取经过处理的mysql数据
                self.selectSql = "select * from (select a.date,a.pdtid,a.country_iso,a.channel_category,a.pid,datediff(str_to_date(a.retain_day,'%Y%m%d'),str_to_date(b.retain_day,'%Y%m%d')) as day_diff,b.retain_day as retain_day_min,log(b.spec_retain/a.spec_retain) as log_retain_rate,b.spec_retain from (select * from ROI_Retained_Predict_GroupBy_c_his where retain_day<" + self.today + " and spec_retain>0 and date>=" + self.daysAgo60 + ") a,(select date,min(retain_day) as retain_day,pdtid,country_iso,channel_category,pid,spec_retain as spec_retain from ROI_Retained_Predict_GroupBy_c_his where retain_day<" + self.today + " and spec_retain>0 and date>=" + self.daysAgo60 + " group by date,pdtid,country_iso,channel_category,pid) b where a.date=b.date and a.pdtid=b.pdtid and a.country_iso=b.country_iso and a.channel_category=b.channel_category and a.pid=b.pid) d where day_diff>0 and log_retain_rate is not null;"
                self.predictSelectSql = "select b.date,b.retain_day,b.pdtid,b.country_iso,b.channel_category,b.pid,b.spec_retain,datediff(str_to_date(b.retain_day,'%Y%m%d'),str_to_date(b.date,'%Y%m%d')) as day_diff from (select date,max(retain_day) as retain_day,pdtid,country_iso,channel_category,pid from ROI_Retained_Predict_GroupBy_c_his where retain_day<" + self.today + " and spec_retain>0 and date>=" + self.daysAgo60 + " group by date,pdtid,country_iso,channel_category,pid) a,ROI_Retained_Predict_GroupBy_c_his b where a.date=b.date and a.pdtid=b.pdtid and a.country_iso=b.country_iso and a.channel_category=b.channel_category and a.pid=b.pid and a.retain_day=b.retain_day;"


        elif self.table == "ROI_Purchase_Predict_GroupBy_c_his":
            self.exeSql = "INSERT INTO ROI_Purchase_Predict_GroupBy_c_his (`date`, revenue_day, pdtid, country_iso, channel_category, pid,purchase) select date,revenue_day,pdtid,country_iso,channel_category,pid,purchase from (select date,revenue_day,pdtid,country_iso,channel_category,pid,sum(purchase) as purchase from ROI_Purchase where date>="+ self.daysAgo4 +" group by date,revenue_day,pdtid,country_iso,channel_category,pid) t on DUPLICATE KEY UPDATE purchase=t.purchase;"
            self.keyList = [1,2,3,4]
            self.valueList = [5,7]
            ...
            if self.colname == "imp_num":
                ...
                self.insertSql = "INSERT INTO ROI_News_Predict_GroupBy_c_his (`date`,revenue_date,pdtid,country_iso,channel_category,pid,ad_type_id,ad_type,imp_revenue_predict) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s) ON DUPLICATE KEY UPDATE imp_revenue_predict=%s;"
                self.selectSql = "select * from (select a.date,a.pdtid,a.country_iso,a.channel_category,a.pid,a.ad_type_id,a.ad_type,datediff(str_to_date(a.revenue_date,'%Y%m%d'),str_to_date(b.revenue_date,'%Y%m%d')) as day_diff,b.revenue_date as purchase_day_min,log(b.imp_revenue/a.imp_revenue) as log_purchase_rate,b.imp_revenue from (select * from ROI_News_Predict_GroupBy_c_his where revenue_date<" + self.today + " and imp_revenue>0) a,(select date,min(revenue_date) as revenue_date,pdtid,country_iso,channel_category,pid,ad_type_id,ad_type,imp_revenue from ROI_News_Predict_GroupBy_c_his where revenue_date<" + self.today + " and imp_revenue>0 group by date,pdtid,country_iso,channel_category,pid,ad_type_id,ad_type) b where a.date=b.date and a.pdtid=b.pdtid and a.country_iso=b.country_iso and a.channel_category=b.channel_category and a.pid=b.pid and a.ad_type_id=b.ad_type_id and a.ad_type=b.ad_type) d where day_diff>0 and log_purchase_rate is not null;"
                ...
                self.insertSql = "INSERT INTO ROI_News_Predict_GroupBy_c_his (`date`,revenue_date,pdtid,country_iso,channel_category,pid,ad_type_id,ad_type,click_revenue_predict) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s) ON DUPLICATE KEY UPDATE click_revenue_predict=%s;"
                self.selectSql = "select * from (select a.date,a.pdtid,a.country_iso,a.channel_category,a.pid,a.ad_type_id,a.ad_type,datediff(str_to_date(a.revenue_date,'%Y%m%d'),str_to_date(b.revenue_date,'%Y%m%d')) as day_diff,b.revenue_date as purchase_day_min,log(b.click_revenue/a.click_revenue) as log_purchase_rate,b.click_revenue from (select * from ROI_News_Predict_GroupBy_c_his where revenue_date<" + self.today + " and click_revenue>0) a,(select date,min(revenue_date) as revenue_date,pdtid,country_iso,channel_category,pid,ad_type_id,ad_type,click_revenue from ROI_News_Predict_GroupBy_c_his where revenue_date<" + self.today + " and click_revenue>0 group by date,pdtid,country_iso,channel_category,pid,ad_type_id,ad_type) b where a.date=b.date and a.pdtid=b.pdtid and a.country_iso=b.country_iso and a.channel_category=b.channel_category and a.pid=b.pid and a.ad_type_id=b.ad_type_id and a.ad_type=b.ad_type) d where day_diff>0 and log_purchase_rate is not null;"
        ...
        while True:
            ...
            if not rs:
                ...
                for predictDate in predictDateList:
                    print ("######### currentTime is %s") % (datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f"))
                    print predictDate
                    if getPlusDay(predictDate[0],int(predictDate[2])) < self.reg.daysAgo4:
                        continue
                    beginIndex = int(predictDate[2])
                    if beginIndex >= pDays:
                        continue
                    ...
                    for i in range(beginIndex, pDays):
                        ...
                        if self.reg.table == "ROI_Retained_Predict_GroupBy_c_his":
                            resultValues = [predictDate[0], dateStr, keys[0], keys[1], keys[2], keys[3], result, result]
                        elif self.reg.table == "ROI_Purchase_Predict_GroupBy_c_his":
                            resultValues = [predictDate[0], dateStr, keys[0], keys[1], keys[2], keys[3], result, result]
                        elif self.reg.table == "ROI_install_Predict_GroupBy_c_his":
                            resultValues = [dateStr, keys[0], keys[1], keys[2], keys[3], result, result]
                        elif self.reg.table == "ROI_News_Predict_GroupBy_c_his":
                            resultValues = [predictDate[0], dateStr, keys[0], keys[1], keys[2], keys[3], keys[4], keys[5], result, result]
