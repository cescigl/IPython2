...
class ARIMAModel(baseModel):
    ...
    def __init__(self, tablename, whereSql, predictDays, beginDay, endDay, colname):
        ...
        #self.daysAgo4 = getPlusDay(self.today, -4)

        if self.table == "ROI_Retained_Predict_GroupBy_c_zhx":
            self.keyList = [0,2,3,4,5]
            self.valueList = [1,6]
            if self.colname == "retain":
                self.insertSql = "INSERT INTO ROI_Retained_Predict_GroupBy_c_zhx (`date`, retain_day, pdtid, country_iso,channel_category, pid, retain_predict) VALUES (%s,%s,%s,%s,%s,%s,%s) ON DUPLICATE KEY UPDATE retain_predict=%s;"
                self.selectSql = "select date,retain_day,pdtid,country_iso,channel_category,pid,if(1/retain,retain,retain_predict)  as retain from ROI_Retained_Predict_GroupBy_c_zhx where " + whereSql + " and retain_day<" + endDay + " and retain_day>="+ beginDay +";"
            elif self.colname == "spec_retain":
                self.insertSql = "INSERT INTO ROI_Retained_Predict_GroupBy_c_zhx (`date`, retain_day, pdtid, country_iso,channel_category, pid, spec_retain_predict) VALUES (%s,%s,%s,%s,%s,%s,%s) ON DUPLICATE KEY UPDATE spec_retain_predict=%s;"
                self.selectSql = "select date,retain_day,pdtid,country_iso,channel_category,pid,if(1/spec_retain,spec_retain,spec_retain_predict)  as spec_retain from ROI_Retained_Predict_GroupBy_c_zhx where " + whereSql + " and retain_day<" + endDay + " and retain_day>="+ beginDay +";"
            
        elif self.table == "ROI_Purchase_Predict_GroupBy_c_zhx":
            self.keyList = [0,2,3,4,5]
            self.valueList = [1,6]
            self.insertSql = "INSERT INTO ROI_Purchase_Predict_GroupBy_c_zhx (`date`,revenue_day,pdtid,country_iso,channel_category,pid,purchase_predict) VALUES (%s,%s,%s,%s,%s,%s,%s) ON DUPLICATE KEY UPDATE purchase_predict=%s;"
            self.selectSql = "select date,revenue_day,pdtid,country_iso,channel_category,pid,if(1/purchase,purchase,purchase_predict)  as purchase from ROI_Purchase_Predict_GroupBy_c_zhx where " + whereSql + " and revenue_day<" + endDay + " and revenue_day>="+ beginDay +";"
            
        elif self.table == "ROI_News_Predict_GroupBy_c_zhx":
            self.keyList = [0,2,3,4,5,6,7]
            self.valueList = [1,8]

            if self.colname == "imp_num":
                self.insertSql = "INSERT INTO ROI_News_Predict_GroupBy_c_zhx (`date`,revenue_date,pdtid,country_iso,channel_category,pid,ad_type_id,ad_type,imp_num_predict) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s) ON DUPLICATE KEY UPDATE imp_num_predict=%s;"

                self.selectSql = "select `date`,revenue_date,pdtid,country_iso,channel_category,pid,ad_type_id,ad_type,if(1/imp_num,imp_num,imp_num_predict) as imp_num from ROI_News_Predict_GroupBy_c_zhx where " + whereSql + " and revenue_date<" + endDay + " and revenue_date>="+ beginDay +";"
            elif self.colname == "click_num":
                self.insertSql = "INSERT INTO ROI_News_Predict_GroupBy_c_zhx (`date`,revenue_date,pdtid,country_iso,channel_category,pid,ad_type_id,ad_type,click_num_predict) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s) ON DUPLICATE KEY UPDATE click_num_predict=%s;"
                self.selectSql = "select `date`,revenue_date,pdtid,country_iso,channel_category,pid,ad_type_id,ad_type,if(1/click_num,click_num,click_num_predict) as click_num from ROI_News_Predict_GroupBy_c_zhx where " + whereSql + " and revenue_date<" + endDay + " and revenue_date>="+ beginDay +";"
            elif self.colname == "imp_revenue":
                self.insertSql = "INSERT INTO ROI_News_Predict_GroupBy_c_zhx (`date`,revenue_date,pdtid,country_iso,channel_category,pid,ad_type_id,ad_type,imp_revenue_predict) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s) ON DUPLICATE KEY UPDATE imp_revenue_predict=%s;"
                self.selectSql = "select `date`,revenue_date,pdtid,country_iso,channel_category,pid,ad_type_id,ad_type,if(1/imp_revenue,imp_revenue,imp_revenue_predict) as imp_revenue from ROI_News_Predict_GroupBy_c_zhx where " + whereSql + " and revenue_date<" + endDay + " and revenue_date>="+ beginDay +";"
            elif self.colname == "click_revenue":
                self.insertSql = "INSERT INTO ROI_News_Predict_GroupBy_c_zhx (`date`,revenue_date,pdtid,country_iso,channel_category,pid,ad_type_id,ad_type,click_revenue_predict) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s) ON DUPLICATE KEY UPDATE click_revenue_predict=%s;"
                self.selectSql = "select `date`,revenue_date,pdtid,country_iso,channel_category,pid,ad_type_id,ad_type,if(1/click_revenue,click_revenue,click_revenue_predict) as click_revenue from ROI_News_Predict_GroupBy_c_zhx where " + whereSql + " and revenue_date<" + endDay + " and revenue_date>="+ beginDay +";"
            else:
...
class RegModel(baseModel):
    ...
    线性回归model类
    """
    def __init__(self, tablename, predictDays, colname=None):
        """
        参数解释：
        tablename:预测的表名
        predictDays:待预测的天数
        ...
        if self.table == "ROI_Retained_Predict_GroupBy_c_zhx":
            ...
            if self.colname == "retain":
                ...
                self.predictSelectSql = "select b.date,b.retain_day,b.pdtid,b.country_iso,b.channel_category,b.pid,b.retain,datediff(str_to_date(b.retain_day,'%Y%m%d'),str_to_date(b.date,'%Y%m%d')) as day_diff from (select date,max(retain_day) as retain_day,pdtid,country_iso,channel_category,pid from ROI_Retained_Predict_GroupBy_c_zhx where retain_day<" + self.today + " and retain>0 and date>=" + self.daysAgo60 + " group by date,pdtid,country_iso,channel_category,pid) a,ROI_Retained_Predict_GroupBy_c_zhx b where a.date=b.date and a.pdtid=b.pdtid and a.country_iso=b.country_iso and a.channel_category=b.channel_category and a.pid=b.pid and a.retain_day=b.retain_day;"
            elif self.colname == "spec_retain":
                self.insertSql = "INSERT INTO ROI_Retained_Predict_GroupBy_c_zhx (`date`, retain_day, pdtid, country_iso,channel_category, pid,spec_retain_predict) VALUES (%s,%s,%s,%s,%s,%s,%s) ON DUPLICATE KEY UPDATE spec_retain_predict=%s;"
                #获取经过处理的mysql数据
                self.selectSql = "select * from (select a.date,a.pdtid,a.country_iso,a.channel_category,a.pid,datediff(str_to_date(a.retain_day,'%Y%m%d'),str_to_date(b.retain_day,'%Y%m%d')) as day_diff,b.retain_day as retain_day_min,log(b.spec_retain/a.spec_retain) as log_retain_rate,b.spec_retain from (select * from ROI_Retained_Predict_GroupBy_c_zhx where retain_day<" + self.today + " and spec_retain>0 and date>=" + self.daysAgo60 + ") a,(select date,min(retain_day) as retain_day,pdtid,country_iso,channel_category,pid,spec_retain as spec_retain from ROI_Retained_Predict_GroupBy_c_zhx where retain_day<" + self.today + " and spec_retain>0 and date>=" + self.daysAgo60 + " group by date,pdtid,country_iso,channel_category,pid) b where a.date=b.date and a.pdtid=b.pdtid and a.country_iso=b.country_iso and a.channel_category=b.channel_category and a.pid=b.pid) d where day_diff>0 and log_retain_rate is not null;"
                self.predictSelectSql = "select b.date,b.retain_day,b.pdtid,b.country_iso,b.channel_category,b.pid,b.spec_retain,datediff(str_to_date(b.retain_day,'%Y%m%d'),str_to_date(b.date,'%Y%m%d')) as day_diff from (select date,max(retain_day) as retain_day,pdtid,country_iso,channel_category,pid from ROI_Retained_Predict_GroupBy_c_zhx where retain_day<" + self.today + " and spec_retain>0 and date>=" + self.daysAgo60 + " group by date,pdtid,country_iso,channel_category,pid) a,ROI_Retained_Predict_GroupBy_c_zhx b where a.date=b.date and a.pdtid=b.pdtid and a.country_iso=b.country_iso and a.channel_category=b.channel_category and a.pid=b.pid and a.retain_day=b.retain_day;"
        elif self.table == "ROI_Purchase_Predict_GroupBy_c_zhx":
            self.exeSql = "INSERT INTO ROI_Purchase_Predict_GroupBy_c_zhx (`date`, revenue_day, pdtid, country_iso, channel_category, pid, purchase) select date,revenue_day,pdtid,ifnull(country_iso,'') as country_iso,ifnull(channel_category,'') as channel_category,ifnull(pid,'') as pid,purchase from (select date,revenue_day,pdtid,country_iso,channel_category,pid,sum(purchase) as purchase from (select date,revenue_day,pdtid,country_iso,channel_category,pid,sum(purchase) as purchase from ROI_Purchase  where pdtid<>'' and country_iso<>'' and channel_category<>'' and pid<>'' and date>="+ self.daysAgo4 + " group by date,revenue_day,pdtid,country_iso,channel_category,pid) t1 group by date,revenue_day,pdtid,country_iso,channel_category,pid WITH ROLLUP union select date,revenue_day,pdtid,country_iso,channel_category,pid,sum(purchase) as purchase from (select date,revenue_day,pdtid,country_iso,channel_category,pid,sum(purchase) as purchase from ROI_Purchase  where pdtid<>'' and country_iso<>'' and channel_category<>'' and pid<>'' and date>="+ self.daysAgo4 +" group by date,revenue_day,pdtid,country_iso,channel_category,pid) t2 group by date,revenue_day,pdtid,channel_category,pid,country_iso WITH ROLLUP) t3 where pdtid is not NULL on DUPLICATE KEY UPDATE purchase=t3.purchase;;"
            self.keyList = [1,2,3,4]
            self.valueList = [5,7]
            ...
            self.exeSql = "INSERT INTO ROI_News_Predict_GroupBy_c_zhx (`date`,revenue_date,pdtid,country_iso,channel_category,pid,ad_type_id,ad_type,imp_num,click_num,imp_revenue,click_revenue) \
            ...
            if self.colname == "imp_num":
                self.insertSql = "INSERT INTO ROI_News_Predict_GroupBy_c_zhx (`date`,revenue_date,pdtid,country_iso,channel_category,pid,ad_type_id,ad_type,imp_num_predict) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s) ON DUPLICATE KEY UPDATE imp_num_predict=%s;"
                self.selectSql = "select * from (select a.date,a.pdtid,a.country_iso,a.channel_category,a.pid,a.ad_type_id,a.ad_type,datediff(str_to_date(a.revenue_date,'%Y%m%d'),str_to_date(b.revenue_date,'%Y%m%d')) as day_diff,b.revenue_date as purchase_day_min,log(b.imp_num/a.imp_num) as log_purchase_rate,b.imp_num from (select * from ROI_News_Predict_GroupBy_c_zhx where date>=" + self.daysAgo60 + " and revenue_date<" + self.today + " and imp_num>0) a,(select date,min(revenue_date) as revenue_date,pdtid,country_iso,channel_category,pid,ad_type_id,ad_type,imp_num from ROI_News_Predict_GroupBy_c_zhx where date>=" + self.daysAgo60 + " and revenue_date<" + self.today + " and imp_num>0 group by date,pdtid,country_iso,channel_category,pid,ad_type_id,ad_type) b where a.date=b.date and a.pdtid=b.pdtid and a.country_iso=b.country_iso and a.channel_category=b.channel_category and a.pid=b.pid and a.ad_type_id=b.ad_type_id and a.ad_type=b.ad_type) d where day_diff>0 and log_purchase_rate is not null;"
                #测试
                #self.selectSql = "select * from (select a.date,a.pdtid,a.country_iso,a.channel_category,a.pid,a.ad_type_id,a.ad_type,datediff(str_to_date(a.revenue_date,'%Y%m%d'),str_to_date(b.revenue_date,'%Y%m%d')) as day_diff,b.revenue_date as purchase_day_min,log(b.imp_num/a.imp_num) as log_purchase_rate,b.imp_num from (select * from ROI_News_Predict_GroupBy_c_zhx where revenue_date<" + self.today + " and pdtid=600001 and country_iso='ES' and channel_category='Organic' and pid='Organic' and ad_type_id='admob' and ad_type='video' and imp_num>0) a,(select date,min(revenue_date) as revenue_date,pdtid,country_iso,channel_category,pid,ad_type_id,ad_type,imp_num from ROI_News_Predict_GroupBy_c_zhx where revenue_date<" + self.today + " and imp_num>0 group by date,pdtid,country_iso,channel_category,pid,ad_type_id,ad_type) b where a.date=b.date and a.pdtid=b.pdtid and a.country_iso=b.country_iso and a.channel_category=b.channel_category and a.pid=b.pid and a.ad_type_id=b.ad_type_id and a.ad_type=b.ad_type) d where day_diff>0 and log_purchase_rate is not null;"
                self.predictSelectSql = "select b.date,b.revenue_date,b.pdtid,b.country_iso,b.channel_category,b.pid,b.ad_type_id,b.ad_type,b.imp_num,datediff(str_to_date(b.revenue_date,'%Y%m%d'),str_to_date(b.date,'%Y%m%d')) as day_diff from (select date,max(revenue_date) as revenue_date,pdtid,country_iso,channel_category,pid,ad_type_id,ad_type from ROI_News_Predict_GroupBy_c_zhx where date>=" + self.daysAgo60 + " and revenue_date<" + self.today + " and imp_num>0 group by date,pdtid,country_iso,channel_category,pid,ad_type_id,ad_type) a,ROI_News_Predict_GroupBy_c_zhx b where a.date=b.date and a.pdtid=b.pdtid and a.country_iso=b.country_iso and a.channel_category=b.channel_category and a.pid=b.pid and a.revenue_date=b.revenue_date and a.ad_type_id=b.ad_type_id and a.ad_type=b.ad_type;"
            elif self.colname == "click_num":
                self.insertSql = "INSERT INTO ROI_News_Predict_GroupBy_c_zhx (`date`,revenue_date,pdtid,country_iso,channel_category,pid,ad_type_id,ad_type,click_num_predict) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s) ON DUPLICATE KEY UPDATE click_num_predict=%s;"
                self.selectSql = "select * from (select a.date,a.pdtid,a.country_iso,a.channel_category,a.pid,a.ad_type_id,a.ad_type,datediff(str_to_date(a.revenue_date,'%Y%m%d'),str_to_date(b.revenue_date,'%Y%m%d')) as day_diff,b.revenue_date as purchase_day_min,log(b.click_num/a.click_num) as log_purchase_rate,b.click_num from (select * from ROI_News_Predict_GroupBy_c_zhx where date>=" + self.daysAgo60 + " and revenue_date<" + self.today + " and click_num>0) a,(select date,min(revenue_date) as revenue_date,pdtid,country_iso,channel_category,pid,ad_type_id,ad_type,click_num from ROI_News_Predict_GroupBy_c_zhx where date>=" + self.daysAgo60 + " and revenue_date<" + self.today + " and click_num>0 group by date,pdtid,country_iso,channel_category,pid,ad_type_id,ad_type) b where a.date=b.date and a.pdtid=b.pdtid and a.country_iso=b.country_iso and a.channel_category=b.channel_category and a.pid=b.pid and a.ad_type_id=b.ad_type_id and a.ad_type=b.ad_type) d where day_diff>0 and log_purchase_rate is not null;"
                self.predictSelectSql = "select b.date,b.revenue_date,b.pdtid,b.country_iso,b.channel_category,b.pid,b.ad_type_id,b.ad_type,b.click_num,datediff(str_to_date(b.revenue_date,'%Y%m%d'),str_to_date(b.date,'%Y%m%d')) as day_diff from (select date,max(revenue_date) as revenue_date,pdtid,country_iso,channel_category,pid,ad_type_id,ad_type from ROI_News_Predict_GroupBy_c_zhx where date>=" + self.daysAgo60 + " and revenue_date<" + self.today + " and click_num>0 group by date,pdtid,country_iso,channel_category,pid,ad_type_id,ad_type) a,ROI_News_Predict_GroupBy_c_zhx b where a.date=b.date and a.pdtid=b.pdtid and a.country_iso=b.country_iso and a.channel_category=b.channel_category and a.pid=b.pid and a.revenue_date=b.revenue_date and a.ad_type_id=b.ad_type_id and a.ad_type=b.ad_type;"
            elif self.colname == "imp_revenue":
                self.insertSql = "INSERT INTO ROI_News_Predict_GroupBy_c_zhx (`date`,revenue_date,pdtid,country_iso,channel_category,pid,ad_type_id,ad_type,imp_revenue_predict) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s) ON DUPLICATE KEY UPDATE imp_revenue_predict=%s;"
                self.selectSql = "select * from (select a.date,a.pdtid,a.country_iso,a.channel_category,a.pid,a.ad_type_id,a.ad_type,datediff(str_to_date(a.revenue_date,'%Y%m%d'),str_to_date(b.revenue_date,'%Y%m%d')) as day_diff,b.revenue_date as purchase_day_min,log(b.imp_revenue/a.imp_revenue) as log_purchase_rate,b.imp_revenue from (select * from ROI_News_Predict_GroupBy_c_zhx where date>=" + self.daysAgo60 + " and revenue_date<" + self.today + " and imp_revenue>0) a,(select date,min(revenue_date) as revenue_date,pdtid,country_iso,channel_category,pid,ad_type_id,ad_type,imp_revenue from ROI_News_Predict_GroupBy_c_zhx where date>=" + self.daysAgo60 + " and revenue_date<" + self.today + " and imp_revenue>0 group by date,pdtid,country_iso,channel_category,pid,ad_type_id,ad_type) b where a.date=b.date and a.pdtid=b.pdtid and a.country_iso=b.country_iso and a.channel_category=b.channel_category and a.pid=b.pid and a.ad_type_id=b.ad_type_id and a.ad_type=b.ad_type) d where day_diff>0 and log_purchase_rate is not null;"
                self.predictSelectSql = "select b.date,b.revenue_date,b.pdtid,b.country_iso,b.channel_category,b.pid,b.ad_type_id,b.ad_type,b.imp_revenue,datediff(str_to_date(b.revenue_date,'%Y%m%d'),str_to_date(b.date,'%Y%m%d')) as day_diff from (select date,max(revenue_date) as revenue_date,pdtid,country_iso,channel_category,pid,ad_type_id,ad_type from ROI_News_Predict_GroupBy_c_zhx where date>=" + self.daysAgo60 + " and revenue_date<" + self.today + " and imp_revenue>0 group by date,pdtid,country_iso,channel_category,pid,ad_type_id,ad_type) a,ROI_News_Predict_GroupBy_c_zhx b where a.date=b.date and a.pdtid=b.pdtid and a.country_iso=b.country_iso and a.channel_category=b.channel_category and a.pid=b.pid and a.revenue_date=b.revenue_date and a.ad_type_id=b.ad_type_id and a.ad_type=b.ad_type;"
            elif self.colname == "click_revenue":
                self.insertSql = "INSERT INTO ROI_News_Predict_GroupBy_c_zhx (`date`,revenue_date,pdtid,country_iso,channel_category,pid,ad_type_id,ad_type,click_revenue_predict) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s) ON DUPLICATE KEY UPDATE click_revenue_predict=%s;"
                self.selectSql = "select * from (select a.date,a.pdtid,a.country_iso,a.channel_category,a.pid,a.ad_type_id,a.ad_type,datediff(str_to_date(a.revenue_date,'%Y%m%d'),str_to_date(b.revenue_date,'%Y%m%d')) as day_diff,b.revenue_date as purchase_day_min,log(b.click_revenue/a.click_revenue) as log_purchase_rate,b.click_revenue from (select * from ROI_News_Predict_GroupBy_c_zhx where date>=" + self.daysAgo60 + " and revenue_date<" + self.today + " and click_revenue>0) a,(select date,min(revenue_date) as revenue_date,pdtid,country_iso,channel_category,pid,ad_type_id,ad_type,click_revenue from ROI_News_Predict_GroupBy_c_zhx where date>=" + self.daysAgo60 + " and revenue_date<" + self.today + " and click_revenue>0 group by date,pdtid,country_iso,channel_category,pid,ad_type_id,ad_type) b where a.date=b.date and a.pdtid=b.pdtid and a.country_iso=b.country_iso and a.channel_category=b.channel_category and a.pid=b.pid and a.ad_type_id=b.ad_type_id and a.ad_type=b.ad_type) d where day_diff>0 and log_purchase_rate is not null;"
                self.predictSelectSql = "select b.date,b.revenue_date,b.pdtid,b.country_iso,b.channel_category,b.pid,b.ad_type_id,b.ad_type,b.click_revenue,datediff(str_to_date(b.revenue_date,'%Y%m%d'),str_to_date(b.date,'%Y%m%d')) as day_diff from (select date,max(revenue_date) as revenue_date,pdtid,country_iso,channel_category,pid,ad_type_id,ad_type from ROI_News_Predict_GroupBy_c_zhx where date>=" + self.daysAgo60 + " and revenue_date<" + self.today + " and click_revenue>0 group by date,pdtid,country_iso,channel_category,pid,ad_type_id,ad_type) a,ROI_News_Predict_GroupBy_c_zhx b where a.date=b.date and a.pdtid=b.pdtid and a.country_iso=b.country_iso and a.channel_category=b.channel_category and a.pid=b.pid and a.revenue_date=b.revenue_date and a.ad_type_id=b.ad_type_id and a.ad_type=b.ad_type;"
    ...
    def predictResult(self, predictDic, key, keys, pDays, y_predict, cursor):
        for predictDate in predictDic[key]:
            ...
            print predictDate
            if getPlusDay(predictDate[0],int(predictDate[2])) < self.daysAgo4:
                continue
    ...
    def predict(self):
        ...
        #初始化数据
        print self.exeSql
        cursor.execute(self.exeSql)
        cursor.execute("commit;")
        #获取预测的原始数据
        cursor.execute(self.selectSql)
        print self.selectSql
        data = cursor.fetchall()
        cursor.execute(self.predictSelectSql)
        print self.predictSelectSql
        ...
        for key in dataDic:
            ...
            print key
            print len(dataDic[key])
            print len(predictDic[key])
