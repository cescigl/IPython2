...
def load_train():
    ...
    path = os.path.join('./RSdata',  'train_45000.csv')
    #path = os.path.join('./RSdata',  'train.csv')
...
def load_test():
    ...
    path = os.path.join('./RSdata',  'test_5000.csv')
    #path = os.path.join('./RSdata',  'test.csv')
    ...
    while (1):
        ...
        X_test_score.append(int(arr[2]))
    f.close()
    return X_test_uid,X_test_iid,X_test_score
...
X_test_uid,X_test_iid,X_test_score = load_test()
...
X_test_score=np.array(X_test_score)
X_test_score=X_test_score.reshape(X_test_score.shape[0],1)
...
x1=Embedding(output_dim=128, input_dim=75, input_length=1)(input_1)
x2=Embedding(output_dim=128, input_dim=14709, input_length=1)(input_2)
x1=Flatten()(x1)
x2=Flatten()(x2)
...
result = np.hstack((X_test_uid,X_test_iid,X_test_score,Y_test_score))

np.savetxt("output.txt",result,fmt="%d,%d,%d,%f",delimiter=",")
