...
class MeshRecommend(Resource):
    def get(self, arg):
        ...
        if endtime - begintime >750:
            print mesh_id + " REC TIMEOUT."
        print "###### End Rec ##########"
...
class Recommend(object):
    ...
    def filterHaveMeshed(self, user, currentTime):
        ...
        self.log.info('['+self.mesh_id+'] 10s or more meshed user:'+str(last10shaveMeshed))
    ...
    def getUserDetail(self, id):
        ...
        ud['lang'] = userDesDict['lang'] if userDesDict.has_key('lang') else 'es-US'
        return ud
    ...
    def userListSplitDetail(self, l, ld, selfUd):
        ...
        if selfUd['age']<=18:
            ...
            self.log.info('['+self.mesh_id+'] split user same lang#pf#age:'+str(sameLangSamePf))
            self.log.info('['+self.mesh_id+'] split user same lang#age:'+str(sameLangDiffPf))
        ...
        self.log.info('['+self.mesh_id+'] split user same lang#pf:'+str(sameLangSamePf))
        self.log.info('['+self.mesh_id+'] split user same lang:'+str(sameLangDiffPf))
        self.log.info('['+self.mesh_id+'] split user same pf:'+str(diffLangSamePf))
        self.log.info('['+self.mesh_id+'] split user same none:'+str(other))

        return result

    def userListSplit(self, l, ld):
        """
        根据候选用户，根据规则拆分用户
        """
        result = []
    ...
    def getMiddleCandidate(self, key, currentTime):
        ...
        for u in result:
            ...
            userD[u]=ub
        self.log.info('['+self.mesh_id+'] get complete user detail!')
        #self.refreshGender(userD) #为在线用户刷新匹配性别匹配取向
        
        return result, userD

    def getToBeMatchedUser(self, toBeMatchedTemp, currentTime, gender, meshgender):
        """
        ...
        self.log.info('['+self.mesh_id+'] get gender is appropriate user')
        ...
        self.log.info('['+self.mesh_id+'] filter have mesh user 10s or more:'+str(historyFilterLast10s))
    ...
    def makeNewIDRecDetail(self, accompanyChatUser, toBeMatched, currentTime, userD):
        ...
        if onlineMatchUser:
            ulAll = self.userListSplit(onlineMatchUser, userD)
            for ul in ulAll:
                if ul:
    ...
    def makeNewIDRec(self):
        ...
        if gender == 2:
            ...
            if middleCandidate:
                toBeMatched, toBeMatchedLast10s = self.getToBeMatchedUser(middleCandidate ,currentTime, gender, meshgender)
                if toBeMatched:
